...
describe("Invite", () => {
  ...
  it("should allow invitee and inviter to exchange messages", async () => {
    ...
    const bobOwnerPublicKey = getPublicKey(generateSecretKey());
    const { session: bobSession, event } = await invite.accept(
      bobPublicKey,
      bobSecretKey,
      bobOwnerPublicKey,
    );
    ...
    const sendAndExpect = (
      ...
      const receivedMessage = receiver.receiveEvent(
        sendText(sender, message).event,
      );
  ...
  it("should convert between event and Invite correctly", () => {
    ...
    expect(event.tags).toContainEqual([
      "d",
      "double-ratchet/invites/test-device",
    ]);
  ...
  it("waitFor prefers the canonical public invite over device-scoped invites", async () => {
    ...
    const subscribe = (filter: any, onEvent: (event: any) => void) =>
      relay.subscribe(filter, onEvent).close;
    const alicePrivateKey = generateSecretKey();
    const alicePublicKey = getPublicKey(alicePrivateKey);
    ...
    relay.storeAndDeliver(
      finalizeEvent(deviceInvite.getEvent(), alicePrivateKey),
    );

    const publicInvite = Invite.createNew(alicePublicKey, "public");
    publicInvite.createdAt = 11;
    relay.storeAndDeliver(
      finalizeEvent(publicInvite.getEvent(), alicePrivateKey),
    );
  ...
  it("should handle session reinitialization with serialization after invite acceptance", async () => {
    ...
    const firstMessage = aliceSession!.receiveEvent(
      sendText(bobSession, "Hello Alice!").event,
    );
    ...
    const secondMessage = aliceSession.receiveEvent(
      sendText(bobSession, "Can you still hear me?").event,
    );
  ...
  it("should handle multiple invite acceptances", async () => {
    ...
    const { event: bobEvent } = await invite.accept(
      bobPublicKey,
      bobSecretKey,
      bobOwnerPublicKey,
    );
    const { event: charlieEvent } = await invite.accept(
      charliePublicKey,
      charlieSecretKey,
      charlieOwnerPublicKey,
    );
    ...
    const charlieCall = calls.find(
      ([, identity]) => identity === charliePublicKey,
    );
